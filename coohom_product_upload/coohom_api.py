...
class CoohomUploader:
    ...
    def register_user(self, appuid, name, email, max_retries=3, show_debug=False, timeout=30):
        """
        Register a new user with Coohom to get appuid.
        
        Args:
            appuid (str): The appuid for the user
        ...
        for attempt in range(max_retries):
            try:
                ...
                if show_debug:
                    st.write(f"🔍 Registration attempt {attempt + 1}/{max_retries}")
                    st.write(f"- URL: `{url}`")
                    st.write(f"- Appuid: `{appuid}`")
                ...
                if response.status_code == 200:
                    ...
                    if data.get('c') == '0':  # Success
                        ...
                        if show_debug:
                            st.error(f"❌ Registration failed: {data.get('m', 'Unknown error')}")
                        return {
                            'success': False,
                            'error': data.get('m', 'Unknown error'),
                            'error_code': data.get('c'),
                    ...
                    if attempt < max_retries - 1:
                        wait_time = (attempt + 1) * 2
                        if show_debug:
                            st.warning(f"⏱️ Retrying in {wait_time}s...")
                        time.sleep(wait_time)
                        continue
                    else:
                        return {
                ...
                if attempt < max_retries - 1:
                    ...
                    if show_debug:
                        st.error("⏱️ Registration timeout after all retry attempts")
                    return {
                        'success': False,
                        'error': 'Registration timeout after all retry attempts',
                        'endpoint': 'user_registration',
                        'retry_attempts': max_retries
                    }
            except Exception as e:
        ...
        return {
            'success': False,
            'error': 'Registration failed after all retry attempts',
            'endpoint': 'user_registration',
            'retry_attempts': max_retries
        }

    def login_user(self, appuid, max_retries=3, show_debug=False, timeout=30):
        """
        Generate SSO token for user login.
        ...
        Returns:
            dict: API response with SSO token and URL
        """
        for attempt in range(max_retries):
            try:
                timestamp = str(int(time.time() * 1000))
                ...
                # Generate signature with appuid
                sign_string = f"{self.app_secret}{params['appkey']}{appuid}{params['timestamp']}"
                signature = hashlib.md5(sign_string.encode('utf-8')).hexdigest()
                params['sign'] = signature
                ...
                if show_debug:
                    st.write(f"🔍 Login attempt {attempt + 1}/{max_retries}")
                    st.write(f"- URL: `{url}`")
                    st.write(f"- Appuid: `{appuid}`")
                    st.write(f"- Timeout: `{timeout}s`")
                ...
                if response.status_code == 200:
                    ...
                    if data.get('c') == '0':  # Success
                        ...
                        if show_debug:
                            st.error(f"❌ Login failed: {data.get('m', 'Unknown error')}")
                        return {
                            'success': False,
                            'error': data.get('m', 'Unknown error'),
                            'error_code': data.get('c'),
                ...
                if attempt < max_retries - 1:
                    ...
                    if show_debug:
                        st.error("⏱️ Login timeout after all retry attempts")
                    return {
                        'success': False,
                        'error': 'Login timeout after all retry attempts',
                        'endpoint': 'user_login',
                        'retry_attempts': max_retries
                    }
            except Exception as e:
        ...
        return {
            'success': False,
            'error': 'Login failed after all retry attempts',
            'endpoint': 'user_login',
            'retry_attempts': max_retries
        }
...
def load_credentials():
    """Load API credentials from credentials.txt or credential.txt file."""
    # Try credentials.txt first, then credential.txt
    ...
    st.error("Neither credentials.txt nor credential.txt file found!")
    return None
